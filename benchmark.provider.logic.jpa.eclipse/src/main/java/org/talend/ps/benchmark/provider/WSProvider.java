...
import org.talend.ps.benchmark.model.PersistrencyHelper;
...
public class WSProvider implements Provider<Source> {
    ...
    private String jpaActivity;
    
    private PersistrencyHelper pmHelper = new PersistrencyHelper(); 
	...
	public void setJpaActivity(String jpaActivity) {
		this.jpaActivity = jpaActivity;
	}
    ...
    public Source invoke(Source request) {
        try {
        	...
        	historyIn.addEvent(id);

        	if (jpaActivity.equals("persistAndDelete")) {
            	pmHelper.persistAndDelete();
        	} else if (jpaActivity.equals("persist")) {
            	pmHelper.persist(id);
        	} else {
            	pmHelper.select();
        	}

        	historyOut.addEvent(id);
            return request;
